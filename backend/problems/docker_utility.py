...
def ensure_container_running():
    """Ensure the Docker container is always running."""
    try:
...
def remove_code_dir_in_container():
    ...
    try:
        subprocess.run(['docker', 'exec', CONTAINER_NAME, 'rm', '-rf', '/code_dir'], check=True)
...
def copy_files_to_container():
    ...
    subprocess.run(['docker', 'cp', CODE_DIR, f'{CONTAINER_NAME}:/code_dir'], check=True)

def compile_code():
    """Compile the code within the Docker container."""
    try:
        subprocess.run(['docker', 'exec', CONTAINER_NAME, 'g++', '/code_dir/code.cpp', '-o', '/code_dir/code.out'], check=True, stderr=subprocess.PIPE)
        return True
    except subprocess.CalledProcessError as e:
...
def execute_code():
    ...
    try:
        subprocess.run(f'docker exec {CONTAINER_NAME} /bin/sh -c "/code_dir/code.out < /code_dir/input.txt > /code_dir/output.txt"', check=True, shell=True)
        return True
    except subprocess.CalledProcessError as e:
        with open(ERROR_FILE, 'w') as error_file:
            if e.stderr:
...
def retrieve_output():
    ...
    try:
        subprocess.run(['docker', 'cp', f'{CONTAINER_NAME}:/code_dir/output.txt', OUTPUT_FILE], check=True)
    except subprocess.CalledProcessError as e:
...
def copy_output_to_host(output_path):
    ...
    try:
        subprocess.run(['docker', 'cp', f'{CONTAINER_NAME}:/code_dir/output.txt', output_path], check=True)
    except subprocess.CalledProcessError as e:
...
if __name__ == '__main__':
    ...
    """
    input_data = "12 2002"  # Update input data here
    output_path = "output.txt"  # Ensure this matches the path you intend on the host
    main(code, input_data, output_path)
